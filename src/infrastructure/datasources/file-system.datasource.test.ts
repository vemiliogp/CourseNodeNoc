...
describe("file-system.datasource.ts", () => {
  ...
  test("should throw and error if severity level is not defined", async () => {
    const logDataSource = new FileSystemDataSource();
    const customSeverityLabel = "SUPER_MEGA_HIGH" as LogSeverityLevel;

    try {
      await logDataSource.getLogs(customSeverityLabel);
      expect(true).toBeFalsy();
    } catch (error) {
      const errorString = `${error}`;
      expect(errorString).toContain("SUPER_MEGA_HIGH not implemented");
    }
  });
});
